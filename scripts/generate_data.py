...
def main():
  ...
  # Epinions
  # Because www.trustlet.org is unavailable now, the dataset is included in this repository.
  # print('Downloading and extracting Epinions') 
  # epinions_zip = os.path.join(args.output_dir, 'ratings_data.txt.bz2')
  # epinions_dir = os.path.join(args.output_dir, 'epinions/')
  # epinions_file = os.path.join(args.output_dir, 'epinions/ratings_data.txt.bz2')
  # if not os.path.exists(epinions_dir):
  #     os.mkdir(epinions_dir)
  # print('Downloading and extracting Epinions data')
  # urllib.request.urlretrieve(
  #     'http://www.trustlet.org/datasets/downloaded_epinions/ratings_data.txt.bz2',
  #     epinions_zip)
  # with bz2.open(epinions_zip, 'rb') as zipref:
  #   contents_orig = zipref.readlines()
  #   first_line = [contents_orig[0].split(b'\x00')[-1]]
  #   contents = '\n'.join([l.decode('ascii') for l in first_line + contents_orig[1:-1]])
  #   sio = StringIO(contents)
  #   raw_data = pd.read_csv(sio, sep=' ', header=None, names=['userId', 'movieId', 'rating'], dtype=schema)
  # os.remove(epinions_zip)

  ###
  epinions_zip = 'epinions/ratings_data.txt.bz2'
  epinions_dir = os.path.join(args.output_dir, 'epinions/')
  schema = {
    'userId': np.int64,
    'movieId': np.int64,
  ...
  raw_data = raw_data[raw_data.rating > 3.5]
  user_cnt = raw_data.value_counts('userId')
  valid_users = user_cnt[user_cnt >= 20].index
  movie_cnt = raw_data.value_counts('movieId')
  valid_movies = movie_cnt[movie_cnt >= 20].index
  raw_data = raw_data[(raw_data.userId.isin(valid_users)) & (raw_data.movieId.isin(valid_movies))]
  generate_data(
      raw_data, output_dir=epinions_dir, n_heldout_users=500, min_uc=0,
      min_sc=0)
  print('Done processing Epinions.')
